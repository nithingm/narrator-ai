import React, {
  ...
  const ChatInterface: React.FC<ChatInterfaceProps> = ({
    ...
    useEffect(() => {
      ...
      // If systemPrompt is missing, use a default message
    ...
    const getWelcomeMessage = (char: Character, defaultPrompt: string): string => {
      const welcomes: Record<string, string> = {
        Dracula:
          'Good evening. I have been waiting for someone to disturb my slumber. What brings you to my castle at this late hour?',
        Frankenstein:
          'Who approaches? Another mortal come to gaze upon the monster? Yet you do not flee... How curious.',
        Wednesday:
          'You’re here. How unfortunate... for you. But I suppose if you have made it this far, you might as well stay.',
      };
    ...
    const handleSendMessage = async (): Promise<void> => {
      ...
      try {
        ...
        setMessages((prev) => [
          ...prev,
          { role: 'assistant', content: response.data.message, timestamp: new Date().toISOString() },
        ]);
      } catch (error: any) {
